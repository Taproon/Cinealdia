...
namespace Cinealdia
{
    public partial class PeliculaDetalles : Form
    {
        ...
        private void PeliculaDetalles_Load(object sender, EventArgs e)
        {
            ...
            //Cargar Portada
            connect.Open();
            String consultaimagen = "SELECT portada FROM peliculas WHERE idpelicula ='" + idpelicula + "'";
            MySqlCommand comandoimagen = new MySqlCommand(consultaimagen, connect);
            MySqlDataReader lectorimagen = comandoimagen.ExecuteReader();
            while (lectorimagen.Read())
            {
                String cadena = lectorimagen.GetString(0);
                Bitmap pic = new Bitmap(cadena);
                imagen.Image = pic;
