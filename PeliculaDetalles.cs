...
namespace Cinealdia
{
    public partial class PeliculaDetalles : Form
    {
        ...
        private void PeliculaDetalles_Load(object sender, EventArgs e)
        {
            ...
            //Cargar Portada
            connect.Open();
            String consultaportada = "SELECT portada FROM peliculas WHERE idpelicula ='" + idpelicula + "'";
            MySqlCommand comandoportada = new MySqlCommand(consultaportada, connect);
            MySqlDataReader lectorportada = comandoportada.ExecuteReader();
            while (lectorportada.Read())
            {
                imagen.Load(lectorportada.GetString(0));
            }
